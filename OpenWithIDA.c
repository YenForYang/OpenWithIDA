...
int main(int argc, char** argv) {
	...
	const int NTHEADER_SIGNATURE = 0x00004550, TARG_APP = 1;

	// additional information
	STARTUPINFOA        si = {sizeof(STARTUPINFOA)}; /*si.cb, set size.*/
	PROCESS_INFORMATION pi;

	char app32[] = { 'i','d','a','.','e','x','e',' ','"' },
		app64[] = { 'i','d','a','6','4','.','e','x','e',' ','"' };

	const int cmd32len = sizeof app32 + MAX_PATH,
	          cmd64len = sizeof app64 + MAX_PATH;
	// determine 32 bits or 64 bits
	FILE* f = fopen(argv[TARG_APP], "rb");
	if (f == NULL) {
	...
	const int imgState = fgetc(f);

	if (imgState == IS_32BIT) {
